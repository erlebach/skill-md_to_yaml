...
- **Typography & figure scale:** deck metadata **`title_scale`** / **`content_scale`** (~0.75–4.0) split heading-band vs body-band font sizes (see `CSS_CONTROLS.md`). Optional **per-slide** `title_scale` / `content_scale` **replace** the deck values on that slide (they do not stack). Deck **`figure_scale`** (~0.25–4.0) and optional per-slide **`figure_scale`** on **`layout: figure`** scale the figure panel’s image/SVG/Mermaid **max-width** / **max-height** caps.
