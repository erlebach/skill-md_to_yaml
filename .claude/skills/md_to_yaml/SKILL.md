...
- **Display math (`$$...$$`)** — optional deck metadata: **`math_display_scale`** (float, default `1.0`, allowed ~0.75–2.0) scales block-math font size relative to body text; **`math_display_center`** (bool, default `true`) centers display equations. Override per slide in that slide’s frontmatter with the same keys (`null` / omit = inherit from deck).
