...
Most **figure scaling** is plain CSS in the `<style>` block (not only `:root` variables). For **`layout: figure`**, **`section[role="group"].slide-figure`** is clamped to **one** **`100vh`** screen with **`overflow: hidden`**, a **flex** chain (**`min-height: 0`**) on **`.slide-content-area`** → **`.figure-container`** → **`<figure>`** → **`.figure-asset-wrap`**, and **extra** **`padding-bottom: calc(1in + var(--slide-footer-ui-clearance))`** so the **white panel** does **not** run under the **fixed** **`.slide-footer-nav`** (page counter, view scale) in the **1in** margin band. The template also uses **two** **figure** **paths**: **Mermaid** (shrink-wrapped panel + **JS**); **rasters and file SVG** (not Mermaid) add **`figure-asset-wrap--fill`**. In browsers with **`@supports (zoom: 1)`**, **`<img>`** / file **`.diagram-container svg`** use **`zoom: var(--figure-scale)`**; else **`transform: scale`**. The **--fill** panel uses **`max-height: 100%`** (of the flex-limited **`<figure>`**), with **`min(80vh, 100%)`** (or **85vh** figure-only) on **media** — not plain **`92vh`**. Color tokens: `:root[data-theme="dark" | "light"]`. A prose mirror (if present) is under `skills/deck-compile/compiler/templates/md/base.html.j2.md`.
...
**`<img>`** and file **`.diagram-container svg`** under **`--fill`** use **`max-height: min(80vh, 100%)`** (or **`min(85vh, 100%)`** on **figure-only**), relative to the **clamped** **`<figure>`** from the **slide-figure** flex chain. Where **`@supports (zoom: 1)`** is true, they use **`zoom: var(--figure-scale, 1)`** and **`transform: none`**. The **`.figure-asset-wrap--fill`** **panel** uses **`max-height: 100%`**, **`overflow: hidden`**, inside **`section.slide-figure`**, which has **`padding-bottom: calc(1in + var(--slide-footer-ui-clearance, 2.75rem))`** to clear the **fixed** **bottom** **nav** **band**. In Firefox (no **element** `zoom` in this pattern), the template keeps **`transform: scale`** and **flex**-centers the **image** and **`.diagram-container`**.
