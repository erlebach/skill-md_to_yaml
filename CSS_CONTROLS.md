...
When the slide is **not** rendered as Mermaid (e.g. **`.jpeg`**, **`.png`**, **`.svg`** file), [`figure.html.j2`](.claude/skills/md_to_yaml/compiler/templates/figure.html.j2) adds **`figure-asset-wrap--fill`** on **`.figure-asset-wrap`**. The **panel** uses the same **`--fig-panel`** / **`--fig-panel-border`** tokens, with **`max-width: 100%`** of the figure column. To **widen a white** **background** **inside** an **`.svg`**, **edit** the **source** **file** (e.g. expand the **outer** **`rect`**, **adjust** **`viewBox`**, **+96** user units = **+1in** at **96** **user** **units** **per** **inch**).
