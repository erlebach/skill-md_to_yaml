...
**`<img>`** and file **`.diagram-container svg`** under **`--fill`** use **`max-height: 80vh`** (or **85vh** on **figure-only**), **`object-fit: contain`**, and **`zoom: var(--figure-scale, 1)`** when supported, else **`transform: scale`**. The **`.figure-asset-wrap--fill`** **panel** uses **`overflow: visible`** (not **`hidden`**) so scaled paint is not **clipped** by the **panel**; **`section.slide-figure`** still has **`padding-bottom: calc(1in + var(--slide-footer-ui-clearance, 2.75rem))`** to clear the **fixed** **bottom** **nav** **band**. In Firefox (no **element** `zoom` in this pattern), the template keeps **`transform: scale`** and **flex**-centers the **image** and **`.diagram-container`**.
